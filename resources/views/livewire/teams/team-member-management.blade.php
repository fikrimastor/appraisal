    ...
    @if (Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-form-section submit="addTeamMember">
                            ...
                            <div class="relative z-0 mt-1 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer">
                                @foreach ($this->teamRoles as $index => $role)
                                    <button type="button" class="relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-600 {{ $index > 0 ? 'border-t border-gray-200 dark:border-gray-700 focus:border-none rounded-t-none' : '' }} {{ ! $loop->last ? 'rounded-b-none' : '' }}"
                                            wire:click="$set('addTeamMemberForm.role', '{{ $role->uuid }}')">
                                        <div class="{{ isset($addTeamMemberForm->role) && $addTeamMemberForm->role !== $role->uuid ? 'opacity-50' : '' }}">
                                            <!-- Role Name -->
                                            <div class="flex items-center">
                                                <div class="text-sm text-gray-600 dark:text-gray-400 {{ $addTeamMemberForm->role == $role->uuid ? 'font-semibold' : '' }}">
                                                    {{ $role->name }}
                                                </div>

                                                @if ($addTeamMemberForm->role == $role->uuid)
                ...
                <x-slot name="actions">
                    <x-action-message class="me-3" on="teamMemberInvited">
                        {{ __('Invited.') }}
                    </x-action-message>
                        ...
                        @foreach ($team->users->sortBy('name') as $user)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Gate::check('updateTeamMember', $team) && $this->teamRoles->isNotEmpty())
                                        <button class="ms-2 text-sm text-gray-400 underline" wire:click="manageRole('{{ $user->id }}')">
                                            {{ $user->membership->memberRole?->name }}
                                        </button>
                                    @elseif ($this->teamRoles->isNotEmpty())
            ...
            <div class="relative z-0 mt-1 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer">
                @foreach ($this->teamRoles as $index => $role)
                    <button type="button" class="relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-600 {{ $index > 0 ? 'border-t border-gray-200 dark:border-gray-700 focus:border-none rounded-t-none' : '' }} {{ ! $loop->last ? 'rounded-b-none' : '' }}"
                        ...
                        <div class="{{ $currentRole !== $role->uuid ? 'opacity-50' : '' }}">
                            <!-- Role Name -->
                            <div class="flex items-center">
                                <div class="text-sm text-gray-600 dark:text-gray-400 {{ $currentRole == $role->uuid ? 'font-semibold' : '' }}">
                                    {{ $role->name }}
                                </div>
